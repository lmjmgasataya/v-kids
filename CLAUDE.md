...
- `src/app/kids/` — protected list of all registered kids (`page.tsx`), joined with `guardians`; supports `?q=` search (kid/guardian name, `ilike`) and `?sort=&dir=` column sorting via `KidsTable.tsx` header links; `src/components/SearchBox.tsx` is the shared debounced client search box that updates the URL
...
**Photo capture (service team members)** — `src/components/PhotoCapture.tsx` is a client component that drives the device camera directly (`getUserMedia` → live `<video>` preview → user-triggered frame grab onto a `<canvas>` → `canvas.toBlob` → assigned to a hidden `<input type="file">` via `DataTransfer`, so the surrounding `<form>`/Server Action needs no special handling). It falls back to a plain file input only if camera access fails/is denied. Used by both `/register/team` (required, no existing photo) and `/service-team/[id]/edit` (optional — pass `initialPreviewUrl` to show the current photo with a "Retake photo" option). Server-side, `src/app/register/team/actions.ts` and `src/app/service-team/[id]/edit/actions.ts` resize/re-encode the uploaded `File` with `sharp` (`.rotate()` first, to fix phone EXIF orientation), then upload via `uploadPhoto` in `src/lib/storage.ts` (an S3-compatible client pointed at a **private** Backblaze B2 bucket) and store the returned object key (`photoKey`). To display a photo, generate a time-limited link with `getSignedPhotoUrl(key)` — never store or cache a raw public URL, the bucket has no public access. `next.config.ts` raises `experimental.serverActions.bodySizeLimit` to `10mb` to allow the raw (pre-resize) upload through.
